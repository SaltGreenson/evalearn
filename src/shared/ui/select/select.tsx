...
export function Select({
  ...
  const onSelect = (value: string) => () => {
    const { label, ...optionValue } = options[value] ?? initialOptions[value];

    handleSelect(optionValue["value"] as string, optionValue);
    setActiveItem(value);
          ...
          value={
            isInputChanged
              ...
              : options[activeItem]?.label ??
                initialOptions[activeItem]?.label ??
                ""
          }
        ...
        {isOpen && (
          <div className={styles.itemList}>
            ...
            {!Boolean(Object.keys(options).length) && (
              <div className={clsx(styles.item, styles.emptyItem)}>
                <Flex centerH>
